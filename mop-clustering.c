...
int main(int argc, char const *argv[]){
	...
	while(stopCriteria == false){
		...
		if(beta > 0){
			// Iterative gradient descent (Weiszfeld's algorithm)
			...
			for(i=0; i<k; i++)
				for(j=0; j<d; j++)
					centers[i][j] = alpha * means[i][j] + beta * centers[i][j];
		}
		...
		double cost = 0.0, cost2 = 0.0;
		for(i=0; i<n; i++){
			cost += pow(distances[i], 2);
			cost2 += distances[i];
		}
		cost = alpha * cost + beta * cost2;
